...
export const LogoDefaults = {
  ...
  BORDER_COLOR: "000000",
  FONT_SIZE: 24,
  BORDER_RADIUS: 15, //added
  BACKGROUND_COLOR: "000000", //added
  BORDER_WIDTH: 5, //added
  PADDING: 20,
  MARGIN: 10
};
...
class App extends Component {
  ...
  getHighKey = array => {
    ...
    array.forEach(arrayElement => {
      if (arrayElement.key > highKey) highKey = arrayElement.key;
    });
    highKey++;
    return highKey;
  };
  ...
  goToEditScreen = logo => {
    ...
    const nextLogos = this.state.logos.filter(
      testLogo => testLogo.key !== logo.key
    );
  ...
  afterLogosChangeComplete = () => {
    ...
    console.log(
      "App updated currentLogo: " + this.logoToString(this.state.currentLogo)
    );
